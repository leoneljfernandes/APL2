...
void generarPaquetes(string subDirectorio, datosCompartidos &datos, int maxPaquetesAGenerar)
{
    ...
    while (true)
    {
        ...
        datos.cv_producido.wait(lock, [&datos]
                                { return int(datos.buffer.size()) < datos.maxBufferSize || datos.fin; });
...
void procesarPaquetes(string directorioProcesamiento, string subDirectorioProcesamiento, datosCompartidos &datos)
{
    while (true)
    {
        unique_lock<mutex> lock(datos.mtx);
        ...
        try
        {
            fs::path origen = directorioProcesamiento + nombreArchivo;

            // crear subDirectorioProcesamiento si no existe
            if (!fs::exists(subDirectorioProcesamiento))
            {
                fs::create_directory(subDirectorioProcesamiento);
            }

            fs::path destino = subDirectorioProcesamiento + nombreArchivo;
                ...
                if (archivo.is_open())
                {
                    ...
                    while (getline(archivo, linea))
                    {
                        ...
                        if (pos1 != string::npos && pos2 != string::npos)
                        {
                            ...
                            lock_guard<mutex> lock(datos.mtx);
                            datos.cantPaquetesProcesados++;
                            datos.pesosPorSucursal[destinoSucursal - 1] += peso;
                        }
                    }
                    archivo.close();
...
int main(int argc, char *argv[])
{
    ...
    {
        consumidores.emplace_back(procesarPaquetes, directorioProcesamiento, subDirectorioProcesamiento, ref(datosCompartidos));
    }
    ...
    // Mostrar resultados finales - ahora protegido y solo una vez
    ...
    {
        lock_guard<mutex> lock(datosCompartidos.mtx);
        cout << "Pesos por sucursal:" << endl;
        for (size_t i = 0; i < datosCompartidos.pesosPorSucursal.size(); ++i)
        {
            if (datosCompartidos.pesosPorSucursal[i] > 0)
            { // Solo mostrar sucursales con peso
